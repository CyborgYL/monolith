...
| MonolithBlueprint | blueprint | 91 | 89 baseline + 2 v0.14.11 bulk_fill adapter surface actions (`set_cdo_properties`, `describe_cdo_schema`) registered into `blueprint::` for symmetry with the existing `set_cdo_property` / `get_cdo_properties` reads |
...
| MonolithCore | describe | 2 | `schema` + `list_targets`. Returns rich `FSchemaDescriptor` trees (type names, ImportText forms, enum-value lists, clamp ranges, nested children) for the same 12 adapter namespaces as `bulk_fill`. Companion read-only schema introspection. |
| **Total** | | **1317** in-tree active actions across 19 in-tree namespaces (15 module namespaces + 4 framework: `monolith` meta, `level_sequence`, `bulk_fill`, `describe`). 45 town-gen experimental disabled by default → 1362 with `bEnableProceduralTownGen=true`. Without `WITH_COMMONUI`: 1266 (1311 w/ town gen); without `WITH_GBA`: 1182 (1227 w/ town gen); without both: 1131 (1176 w/ town gen). Without `WITH_METASOUND`: 1301 (the 12 v0.14.10 introspection actions become inert; the `audio` bulk_fill adapter's MetaSound paths gate the same). The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1313** in default-active configuration, **1358** with town gen registered. Live total reported by `monolith_discover()` (including sibling plugins): **1540** = 1317 in-tree + 11 `claudedesign` (sibling `MonolithClaudeDesignBridge`) + 158 `inventory` (sibling `MonolithISX`) + 28 `steam` (sibling `MonolithSteamBridge`) + 26 `substance` (sibling `MonolithSubstance`) | Namespace count rose 18 → 19 with `level_sequence` (v0.14.8) and 19 → 19 with v0.14.11 [Unreleased] (`bulk_fill` + `describe` were already counted as Phase 0 dispatchers; this release lights up their adapters but does not add new namespaces). Sibling-plugin namespaces are documented in their own repos and are NOT in the public Monolith release zip — sibling totals are advisory only |

...
Monolith.uplugin
  ...
  MonolithNiagara       — Niagara particle systems, HLSL module/function creation, DI config, event handlers, sim stages, NPC, effect types, scalability, layout, temporal control, stateless-emitter factory (119 actions)
  ...
  MonolithReflectionIntel — Deterministic reflection intelligence layer. v0.17.0 Phase 1 ships the `decision_query` namespace (5 actions) — markdown decision-record mining into `EngineSource.db` with a lazy bootstrap + hot-reload refresh. Phase 2 risk / Phase 3 cppreflect / Phase 4 network are planned WISHLIST.
```
...
| All others (17 Editor + 1 Runtime) | Default | Editor (MonolithBABridge is optional — empty shell when Blueprint Assist absent). MonolithReflectionIntel ships v0.17.0 (Phase 1 of Reflection Intelligence — `decision_query` namespace). MonolithAudioRuntime is Type: Runtime — provides `UMonolithSoundPerceptionUserData` + `UMonolithAudioPerceptionSubsystem` for `audio::bind_sound_to_perception` |
...
| 3.18 | MonolithReflectionIntel | [specs/SPEC_MonolithReflectionIntel.md](specs/SPEC_MonolithReflectionIntel.md) | Deterministic, $0-LLM reflection intelligence — markdown decision-record mining (Phase 1, v0.17.0, 5 `decision_query` actions). Phase 2 risk, Phase 3 cppreflect, Phase 4 network all WISHLIST. |
...
| **Total** | | **1363** in-tree active actions across 20 in-tree namespaces (16 module namespaces + 4 framework: `monolith` meta, `level_sequence`, `bulk_fill`, `describe`). v0.17.0 adds `decision` (5 actions) from the new `MonolithReflectionIntel` module — namespace count 19 → 20, in-tree total 1358 → 1363. Reconciled 2026-05-28: the per-namespace rows above are authoritative; the prior deferred-batch list (+1/+3/+4/+1/+17/+4/+9/+1 = +40 over the stale 1318 baseline) is now folded in. 45 town-gen experimental actions disabled by default → 1408 with `bEnableProceduralTownGen=true`. Without `WITH_COMMONUI`: 1308 (1353 w/ town gen — strips the 51 CommonUI actions then adds back the unconditional 11); without `WITH_GBA`: 1228 (1273 w/ town gen — strips the 135 GAS actions); without both: 1173 (1218 w/ town gen). Without `WITH_METASOUND`: 1351 (the 12 audio MetaSound introspection actions become inert; the `audio` bulk_fill adapter's MetaSound paths gate the same). The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1359** in default-active configuration, **1404** with town gen registered. **Sibling plugins are NOT counted in the totals above** (per `monolith-release.md` Action Count Discipline) — the public Monolith release zip does not include them. For transparency, live `monolith_discover()` (which includes siblings if installed) returns **1586** = 1363 in-tree + 11 `claudedesign` (`MonolithClaudeDesignBridge`) + 158 `inventory` (`MonolithISX`) + 28 `steam` (`MonolithSteamBridge`) + 26 `substance` (`MonolithSubstance`); the 223 sibling actions are advisory only and never affect the public-release headline. | Namespace count rose 18 → 19 with `level_sequence` (v0.14.8); v0.14.11 [Unreleased] lit up the existing `bulk_fill` + `describe` dispatchers' adapters without adding new namespaces; v0.17.0 brings the `decision` namespace from MonolithReflectionIntel for 19 → 20. The 2026-05-22 `monolith_guide` addition stayed inside the existing `monolith` meta-namespace. Sibling-plugin namespaces are documented in their own repos and are NOT in the public Monolith release zip — sibling totals are advisory only |

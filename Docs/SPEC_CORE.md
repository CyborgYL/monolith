...
Monolith.uplugin
  ...
  MonolithNiagara       — Niagara particle systems, HLSL module/function creation, DI config, event handlers, sim stages, NPC, effect types, scalability, layout, temporal control, stateless-emitter factory, search & discovery (129 actions)
...
| 3.5 | MonolithNiagara | [specs/SPEC_MonolithNiagara.md](specs/SPEC_MonolithNiagara.md) | Niagara particle systems, HLSL module/function, DI config, event handlers, sim stages, layout, temporal control, stateless-emitter factory, search & discovery (129 actions) |
...
>
> **Ballpark, current:** **~1650 actions across ~30 namespaces** total when sibling plugins are loaded; **~1450 public in-tree actions across ~25 namespaces** with sibling namespaces excluded (sibling plugins are not in the public release zip — see Action Count Discipline in `monolith-release.md`).
...
| **Total** | | **1424** in-tree active actions across 26 in-tree namespaces (18 module namespaces — `animation` now also owns the `WITH_CHOOSER`-gated `chooser` namespace — + 8 framework: `monolith` meta, `level_sequence`, `bulk_fill`, `describe`, plus the six Reflection Intel namespaces — `decision`, `risk`, `cppreflect`, `network`, `pipeline`, `reflect`). v0.17.0 adds `decision` (5 actions, Phase 1) + `risk` (5 actions, Phase 2) + `cppreflect` (6 actions, Phase 3a + an [Unreleased] `list_class_specifiers` follow-up) + `network` (4 actions, Phase 4a) + `pipeline` (2 actions, Phase 4a) + 1 `source` audit action (Phase 2 cross-namespace registration `audit_module_dep_reality`) + 4 cross-namespace audit actions on `material` / `niagara` / `blueprint` / `project` (Phase 4a) — namespace count 19 → 24, in-tree total 1358 → 1384 (Phases 1+2+3a = +16; Phase 4a = +10 actions: 4 network + 2 pipeline + 4 cross-namespace audits), then [Unreleased] 1384 → 1385 (+1 cppreflect `list_class_specifiers`), then [Unreleased] 1385 → 1386 (+1 new `reflect` namespace with `rebuild_reflection_index` — the network-completeness workstream; namespace count 24 → 25). Reconciled 2026-05-28: the per-namespace rows above are authoritative; the prior deferred-batch list (+1/+3/+4/+1/+17/+4/+9/+1 = +40 over the stale 1318 baseline) is now folded in. Reconciled 2026-06-05: the headline matched the per-namespace rows after the test/profiling-harness pass Wave 2 (+11: chooser +5 new namespace, animation +5 graph-surgery, editor +1 `list_errored_blueprints`) layered on top of Wave 1 (+12: editor +7, ai +2, project +3). Reconciled 2026-06-06: harness pass Wave 16 (+5: chooser +1 `set_evaluate_chooser_result_reference`, animation +3 `create_state_machine`/`build_state_machine`/`sample_pie_anim_instance`, editor +1 `capture_anim_frames`) brings the headline ~1424. The 2026-06-07 gap-closure pass adds 3 actions (`animation.get_anim_graph_choosers`, `animation.get_transition_rule`, `editor.author_map_settings`) plus param/behavior additions — headline ~1427 in-tree. The 2026-06-07 Motion Matching AI-wander pack then adds ~28 more (animation +19, chooser +4, blueprint +6; plus 3 BT task classes + 1 AIController as runtime classes with no MCP-action delta) — headline ~1450 in-tree. Exact integers are no longer tracked to the unit (see the approximate-count note at the top of this section); query `monolith_discover()` for the live figure. 45 town-gen experimental actions disabled by default → 1469 with `bEnableProceduralTownGen=true`. The 6 `chooser` actions are `WITH_CHOOSER`-gated; without the Chooser plugin: 1418 (the namespace registers nothing). Without `WITH_COMMONUI`: 1361 (1406 w/ town gen — strips the 51 CommonUI actions then adds back the unconditional 11); without `WITH_GBA`: 1281 (1326 w/ town gen — strips the 135 GAS actions); without both: 1226 (1271 w/ town gen). Without `WITH_METASOUND`: 1404 (the 12 audio MetaSound introspection actions become inert; the `audio` bulk_fill adapter's MetaSound paths gate the same). The `ui` namespace double-counts 4 aliased GAS actions; **distinct** action handlers = **1419** in default-active configuration, **1464** with town gen registered. **Sibling plugins are NOT counted in the totals above** (per `monolith-release.md` Action Count Discipline) — the public Monolith release zip does not include them. For transparency, live `monolith_discover()` / `monolith_status` (which includes siblings if installed) returns **1647** = 1424 in-tree + 11 `claudedesign` (`MonolithClaudeDesignBridge`) + 158 `inventory` (`MonolithISX`) + 28 `steam` (`MonolithSteamBridge`) + 26 `substance` (`MonolithSubstance`); the 223 sibling actions are advisory only and never affect the public-release headline. | Namespace count rose 18 → 19 with `level_sequence` (v0.14.8); v0.14.11 [Unreleased] lit up the existing `bulk_fill` + `describe` dispatchers' adapters without adding new namespaces; v0.17.0 Phases 1+2+3a brought three new namespaces from MonolithReflectionIntel — `decision`, `risk`, `cppreflect` — for 19 → 22, plus a +1 audit on the existing `source` namespace. v0.17.0 Phase 4a brings two more — `network`, `pipeline` — for 22 → 24, plus 4 cross-namespace audit actions on existing `material` / `niagara` / `blueprint` / `project` namespaces. [Unreleased] adds the `reflect` namespace (network-completeness workstream) for 24 → 25. The test/profiling-harness pass Wave 2 adds the `WITH_CHOOSER`-gated `chooser` namespace (registered from `MonolithAnimation`) for 25 → 26 in-tree (29 → 30 total). The 2026-05-22 `monolith_guide` addition stayed inside the existing `monolith` meta-namespace. Sibling-plugin namespaces are documented in their own repos and are NOT in the public Monolith release zip — sibling totals are advisory only |

...
Huge thanks to **@alakangas** for both PRs ([#17](https://github.com/tumourlove/monolith/pull/17) memory helper + indexer retrofits, [#18](https://github.com/tumourlove/monolith/pull/18) MetaSound indexer + introspection actions). Additional thanks to **@MaxenceEpitech** for [PR #54](https://github.com/tumourlove/monolith/pull/54) — bone-track discovery action (`list_bone_tracks`), the deprecated-API rewrite of `get_bone_track_keys`, and the PIE/console action triplet (`start_pie`, `stop_pie`, `run_console_command`). The PR work is preserved with author attribution on the merge commits.

...
- **Action count delta vs v0.14.10: `blueprint` +2 (`set_cdo_properties`, `describe_cdo_schema`), `ui` +12 (Tier 2 +8 + Tier 3 +4), `bulk_fill` namespace +2 (Phase 0 dispatchers — `apply`, `list_namespaces`), `describe` namespace +2 (Phase 0 dispatchers — `schema`, `list_targets`).** Live verified via `monolith_discover()` on 2026-05-11: total 1540 actions across 23 namespaces (in-tree: 1317 actions across 19 namespaces; sibling-plugin total 223 = `claudedesign` 11 + `inventory` 158 + `steam` 28 + `substance` 26 — siblings are NOT in the public release zip). Per-namespace breakdown in [`Docs/SPEC_CORE.md` §12](Docs/SPEC_CORE.md#12-action-count-summary).
